...
impl TaskManager {
    ...
    fn inc_syscall_times(&self, id: usize) {
        let mut inner = self.inner.exclusive_access();
        let current = inner.current_task;
        inner.tasks[current].task_syscall_times[id] += 1; // do it atmoic?
    }
...
pub fn get_current_task_status() -> TaskStatus {
    TASK_MANAGER.get_current_task_info().task_status
}

pub fn get_current_task_st_time() -> usize {
    TASK_MANAGER.get_current_task_info().task_st_time
}

pub fn get_current_task_syscall_times() -> [u32; MAX_SYSCALL_NUM] {
    TASK_MANAGER.get_current_task_info().task_syscall_times
}
